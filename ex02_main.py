...
def sample_and_save_images(n_images, image_size, diffusor, model, device, store_path, reverse_transform):
    ...
    plt.savefig(os.path.join(store_path, "sample_image.png"))
...
def run(args):
    ...
    dataset = datasets.CIFAR10(r'E:\MS_AI\Sem-III\ADL\Exercises\exercise_1\cifar10\cifar-10-batches-py', download=True, train=True, transform=transform)
    ...
    testset = datasets.CIFAR10(r'E:\MS_AI\Sem-III\ADL\Exercises\exercise_1\cifar10\cifar-10-batches-py', download=True, train=False, transform=transform)
    testloader = DataLoader(testset, batch_size=int(batch_size/2), shuffle=True)
    save_path = r"E:\MS_AI\Sem-III\ADL\Exercises\ex02_code_skeleton\outputs\sample_images"  
    n_images = 8
    ...
    sample_and_save_images(n_images, image_size, diffusor, model, device, save_path, reverse_transform)
    torch.save(model.state_dict(), os.path.join(r"E:\MS_AI\Sem-III\ADL\Exercises\ex02_code_skeleton\outputs", args.run_name, f"linear_schedule_diffusion.pt"))
